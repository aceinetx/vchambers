...
void vc::VChambers::initalize() {
	...
	if (!data["aim_sensivity"].is_number_float()) {
		...
		aim_sensivity = data["aim_sensivity"].template get<float>();
	}

	if (!data["px"].is_number()) {
		data["px"] = player.x;
	} else {
		player.x = data["px"].template get<int>();
	}

	if (!data["py"].is_number_integer()) {
		data["py"] = player.y;
	} else {
		player.y = data["py"].template get<int>();
	}

	if (!data["ammo"].is_number_integer()) {
		data["ammo"] = 0;
	} else {
		player.ammo = data["ammo"].template get<int>();
	}

	if (!data["level"].is_number_integer()) {
		data["level"] = 1;
	} else {
		level = data["level"].template get<int>();
	}
...
void vc::VChambers::init_level(bool first_init) {
	if (!first_init) {
		entity_list.clear();
		door_list.clear();
		item_list.clear();
...
bool vc::VChambers::collides(char tile, int y, int x) {
...
void vc::VChambers::game_thread() {
	while (true) {
		if (scene == 1) {
			...
			for (EntityTile& et : entitytile_list) {
				if (et.health <= 0)
					entitytile_list.erase(entitytile_list.begin() + i);
				i++;
			}
...
string vc::VChambers::health_bar(int health) {
	string buf;
			...
			while (render_distance < render_range) {
				...
				for (Door& door : door_list) {
					if (door.x == render_ray.x && (door.y == render_ray.y)) {
						if (door.opened == false)
							mvprintw(door.y, door.x, "%c", DOOR::DOOR_CLOSED);
						else if (door.opened == true)
							mvprintw(door.y, door.x, "%c", DOOR::DOOR_OPEN);
						else {
